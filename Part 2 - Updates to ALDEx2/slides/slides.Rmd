...
1. Draw samples of $\mathbf{W}^{\parallel}$ from a measurement model \textcolor{gray}{(can depend on $Y$)}.

2. Draw samples of $W^{\perp}$ from a scale model \textcolor{gray}{(can depend on $\mathbf{W}^\parallel$)}.
...
2. For any value of $\gamma > 0$, it models potential error in the CLR assumption \textcolor{gray}{(false positives will decrease compared to the CLR normalization.)}
...
\pause 
- With 95% certainty, the value of $\theta^\perp$ is within $\pm 2 \gamma$ of the value of $\theta^\perp_{\text{CLR}}$.
...
\textcolor{gray}{With 95% certainty, the true difference in scales falls within the the range $2^{\theta_{\text{CLR}}^\perp \pm 2 \gamma}$.}

\pause

- Suppose that we are performing differential abundance in a case/control study where $\theta_{\text{CLR}} = 0.04$.
...
- Then, this implies that, with 95% certainty, we believe that the scale of the case condition is within a factor of $[2^{\theta_{\text{CLR}}^\perp - 2 \gamma}, 2^{\theta_{\text{CLR}}^\perp + 2 \gamma}] = [0.51,2.05]$ of the control condition.
...
scale_samps <- aldex.makeScaleMatrix(gamma = .15, 
                                     mu = scales, 
                                     conditions = conds,
                                     log=FALSE)
...
scale.cd <- 2^matrix(rnorm(1000*29, 
                           mean = log2(.7), sd = .125), nrow = 29)
scale.control <- 2^matrix(rnorm(1000*66, 
                                mean = log2(1), sd = .125), nrow = 66)
