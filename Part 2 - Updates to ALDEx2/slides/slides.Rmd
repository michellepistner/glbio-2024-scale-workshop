...
- $\mathbf{W}$: depends on both the composition ($W_{dn}^\parallel$) and system scale ($W_n^\perp$):

$$\mathbf{W}_{dn} = \mathbf{W}_{dn}^\parallel W_n^\perp$$
$$W_n^\perp = \sum_{d=1}^D \mathbf{W}_{dn}$$
...
\theta_d &= \text{mean}_{\text{case}}(\log \mathbf{W}_{dn}) - \text{mean}_{\text{control}}(\log \mathbf{W}_{dn})\\
&= ... \\
&= (\text{mean}_{\text{case}}(\log \mathbf{W}_{dn}^\parallel) - \text{mean}_{\text{control}}(\log \mathbf{W}_{dn}^\parallel))\\
...
**Empirical Rule:** 95\% of samples of $\Lambda^\perp$ fall within a factor of $\pm 2 \gamma$.
...
\begin{align*}
\log W_n^\perp = \text{mean}(\log \mathbf{W}_{\cdot n}^\parallel) = \text{mean} (\text{GM}( \mathbf{W}_{\cdot n}^\parallel)).
\end{align*}
...
1. The value of $\theta^\perp$ is within $\pm 2 \gamma$ of the value of $\theta^\perp$ implied by the CLR.

2. The true difference in scales falls within the the range $2^{\theta^\perp \pm 2 \gamma}$ with 95% certainty.
